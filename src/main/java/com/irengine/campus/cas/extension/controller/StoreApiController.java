...
public class StoreApiController {
	...
	public void sendRedirect(@PathVariable("id") Long id,
		...
		String url = "http://vps1.taoware.com:8080/jc/html5/shop_page/shop.html?storeName="
				+ storeName
				+ "&address="
				+ address
				+ "&summary="
	...
	public ResponseEntity<?> create(
			@RequestParam("storeName") String storeName1,
		...
		try {
			...
			introduction = new String(introduction1.getBytes("ISO-8859-1"),
					"utf-8");
		...
		Store store = new Store(storeName, summary, address, province, phone,
				introduction, type);
		...
		if (file != null) {
			...
			try {
				files1 = utilityService.uploadFiles("store", maxId, files,
						request, "");
			} catch (IOException e) {
				return new ResponseEntity<>(new Result<Information>(
						"upload file error", null), HttpStatus.BAD_REQUEST);
			}
			if (files1.size() > 0) {
	...
	public ResponseEntity<?> findByType(
			...
			@RequestParam(value = "province", required = false) String province1) throws UnsupportedEncodingException {
		List<Store> stores = new ArrayList<Store>();
		if (type != null && !"".equals(type)) {
			/* 按类型搜索(最多返回四条) */
			List<Store> stores1 = storeService.findByType(type);
			if (stores1.size() > 4) {
				for (int i = 0; i < 4; i++) {
					stores.add(stores1.get(i));
				}
			} else {
				stores = stores1;
			}
	...
	public ResponseEntity<?> modifyAvatar(@PathVariable("id") long id,
		...
		if (!file.isEmpty()) {
			files.add(file);
			try {
				files1 = utilityService.uploadFiles("store", id, files,
						request, "");
				if (files1.size() > 0) {
					store.setPhoto(files1.get(0));
					storeService.update(store);
