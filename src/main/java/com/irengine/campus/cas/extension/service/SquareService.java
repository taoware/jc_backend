...
public class SquareService {
	...
	public List<Square> findByUserIdAndPermissions(Long userId,
		...
		if (units == null || units.size() < 1) {
			...
			for (Unit unit : units) {
				squares.addAll(findByUnitIdAndPermissionIds(unit.getId(),
						permissionIds));
			}
			return squares;
		}
	...
	private List<Square> findByUnitIdAndPermissionIds(Long unitId,
		...
		List<Square> squares1 = new ArrayList<Square>(squares);
		return squares1;
	}

	private List<Square> findByUnitIdAndType(Long unitId, String type) {
		/* 弊端:不能查找子级的unit对应的广场 */
		List<Square> squares = squareRepository.findByUnitIdAndType(unitId,
				type);
		return squares;
	}

	@SuppressWarnings("unused")
	private List<Square> findByUnitIdAndType2(Long leftId, Long rightId,
			String type) {
		List<Square> squares = squareRepository.findByUnitIdAndType2(leftId,
				rightId, type);
		return squares;
	}
}
